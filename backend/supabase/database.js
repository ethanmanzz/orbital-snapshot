...
export const setupRealTimeSubscriptionForImages = async (userId, date, updateCallback) => {
  ...
  const channel = supabase
    .channel('realtime:public:daily_intake')
    .on('postgres_changes', {
      event: '*',  
      schema: 'public',
      table: 'daily_intake',
    ...
    .subscribe((status) => {
      console.log('Subscription status:', status); // Debugging log for subscription status
      if (status === 'SUBSCRIBED') {
        console.log('Subscription to real-time updates is successful.');
      } else if (status === 'ERROR') {
        console.log('Error subscribing to real-time updates.');
      }
    });
