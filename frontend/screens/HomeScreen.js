...
const HomeScreen = ({ navigation }) => {
  ...
  const handleDateChange = (event, selectedDate) => {
    ...
    fetchUserImages(currentDate); // Fetch images for the selected date
  };
  ...
  //To display the images the user has uploaded
  const fetchUserImages = async (selectedDate) => {
    const data = await fetchUserImagesForDate(selectedDate);
  ...
  useEffect(() => {
    ...
    fetchNutritionData(today);
    fetchWaterIntake(today);
    fetchUserImages(today); // Fetch images for today's date
  }, []);
  ...
  const renderMealSections = () => {
    console.log('Number of images:', userImages.length); // Debug log to check image array length
    return mealTypes.map((mealType, index) => {
      const imageIndex = userImages.length - 1 - index; // Start from the last index
      const imageUrl = imageIndex >= 0 && imageIndex < userImages.length ? userImages[imageIndex] : null; // Get image URL or null if not available
      console.log(`Rendering ${mealType} with image index ${imageIndex} and image URL ${imageUrl}`); // Debug log for each meal section
      return (
        <View key={mealType} style={styles.mealContainer}>
          ...
          {imageUrl ? (
            ...
            <Text>No image available</Text> // Placeholder text for when no image is available
          )}
        </View>
      );
    });
  ...
  const takePhotoFromCamera = async () => {
    try {
      ...
      if (!cameraResp.canceled) {
        ...
        setPhotoUri(uri);
        navigation.navigate('UploadScreen', { photoUri: uri });
